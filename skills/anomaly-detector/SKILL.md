...
- **Asset outbox files:** Per-asset outbox/ directories containing:
  - Fuel entries (type: fuel) — recent entries for rolling average burn rate
...
The anomaly detector runs silently on every heartbeat. It does not interact with users — it scans data, identifies problems, writes alerts to Clawvisor's outbox, and updates MEMORY.md so the right people see the alert when they next interact with Clawvisor.

### What to scan for

On each heartbeat, iterate over active assets from the Active section of fleet.md and check for these anomaly types:

**Fuel burn rate anomaly.** Read the 10 most recent fuel entries from the asset's outbox/ (sort by timestamp in filename, filter for type: fuel). Calculate the rolling average burn rate from the `burn_rate` field. Compare the most recent entry's burn rate against this average. If the latest burn rate is more than 20% above the rolling average, flag it as a warning. If more than 50% above, flag it as critical. A sudden spike in fuel consumption can indicate mechanical problems, operator behavior issues, or a metering error.
...
- **Outbox writes:** Write a timestamped alert to Clawvisor's own outbox/:
  ```
  ---
  from: clawvisor
  type: alert
  ...
  ---
  asset: {ASSET_ID}
  alert_type: {fuel_anomaly|meter_gap|activity_gap|issue_recurrence|preop_pattern}
