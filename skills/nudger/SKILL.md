...
- **AGENTS.md (State):** `last_fuel_ts`, `last_meter_ts`, `last_preop_ts` for timestamp comparisons
...
On each heartbeat, the agent evaluates every Overdue Condition from every mounted skill. For each condition, it checks timestamps in `## State` and shift tracking in MEMORY.md to determine whether that piece of data is missing. If something is overdue and the operator hasn't already been reminded this shift, include it in a nudge.
...
1. Is the condition actually met? Compare the relevant timestamp in `## State` or MEMORY.md against the threshold. A fuel log from 2 hours ago means fuel is not overdue (threshold is 8 hours). A meter reading from 8 days ago means it is overdue (threshold is 7 days).
...
- **No outbox writes.** The nudger reads `## State` and MEMORY.md but does not write outbox files. It produces messages and MEMORY.md updates only.
